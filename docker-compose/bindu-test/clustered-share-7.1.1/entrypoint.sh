...
if [[ "$DISABLE_SHARE" != "true" ]]; then
  # BINDU - create a loadbalancer named share that is sticky based on ip hash
  sed -i s%"server {"%"upstream share {\n        ip_hash;\n        server share1:8080;\n        server share2:8080;\n    }\n\n    server {"%g /etc/nginx/nginx.conf
  # BINDU - removed port from proxy_pass as it is defined in the upstream server entries
